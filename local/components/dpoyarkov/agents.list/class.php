...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function executeComponent(): void
    {
        ...
        if ($this->cache->initCache(
            ...
            $arTypeAgents = self::getFieldListValue($arHlblock, 'UF_TYPEOFACTIVITY'); // получить массив со значениями списочного свойства Виды деятельности агентов
    ...
    private static function getEntityDataClassById(array $arHlblock): string
    {
        ...
        $entity = Bitrix\Highloadblock\HighloadBlockTable::compileEntity($arHlblock);
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $nav->allowAllRecords(true)
            ->setPageSize($this->arParams['AGENTS_COUNT']) //Нужно передать параметр Количество элементов из массива $this->arParams
            ->initFromUri();
        ...
        while ($arAgent = $rsAgents->fetch()) {
             ...
             */
            $arAgent["UF_TYPEOFACTIVITY"] = $arTypeAgents[$arAgent["UF_TYPEOFACTIVITY"]];
             /*
    ...
    public function clickStarAction($agentID)
    {
         ...
         */
         $userOption = CUserOptions::GetOption("mcart_agent", "options_agents_star", false, false);
         /*
        ...
        CUserOptions::SetOption("mcart_agent", "options_agents_star", $value, false, false);
         /*
